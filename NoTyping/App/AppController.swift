...
final class AppController: NSObject, ObservableObject, HotkeyManagerDelegate {
    ...
    private func handlePipelineEvent(_ event: PipelineEvent) {
        ...
        case .transcriptionResult(_, let finalText):
            ...
            hudController.show(state: "\u{2713}", detail: preview, dismissible: true)
            // No auto-hide. User dismisses manually.
        ...
        case .error(let err):
            hudController.show(state: "\u{26A0}", detail: err.errorDescription ?? "Unknown error", isError: true, dismissible: true)
            // No auto-hide. User dismisses manually.
        }
    }
